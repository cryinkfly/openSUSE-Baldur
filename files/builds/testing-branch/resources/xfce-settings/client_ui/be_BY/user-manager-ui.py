...
class MainWindow(Gtk.Window):
    def __init__(self):
        ...
        label_title.set_markup(
            "<big><b>Налады дыспетчара карыстальнікаў</b></big>"
        )
        label_title.set_justify(Gtk.Justification.CENTER)
        info_box.pack_start(label_title, True, True, 0)
        label_info = Gtk.Label(label="У гэтай вобласці вы можаце змяніць налады ўліковага запісу карыстальніка і дадаць/выдаліць карыстальнікаў, звязаных з іх уліковым запісам. Проста выберыце са спісу карыстальніка, якога хочаце змяніць.")
        ...
        column_toggle = Gtk.TreeViewColumn("Выберыце", renderer_toggle, active=0)
        treeview.append_column(column_toggle)

        renderer_text = Gtk.CellRendererText()
        column_text = Gtk.TreeViewColumn("Карыстальнік", renderer_text, text=1)
        treeview.append_column(column_text)

        # Buttons
        close_button = Gtk.Button(label="◀️ Назад")
        close_button.connect("clicked", self.on_close_clicked)

        add_button = Gtk.Button(label="🎭 Ствараць")
        add_button.connect("clicked", self.on_add_clicked)

        configure_button = Gtk.Button(label="⚙️ Наладзіць")
        configure_button.connect("clicked", self.on_configure_clicked)

        del_button = Gtk.Button(label="🗑 Выдаліць")
    ...
    def on_del_clicked(self, button):
        ...
        if selected_option:
            del_selected_user_cmd=f"""
                    ...
                    echo -n {selected_option} > /tmp/_selected_user.XXXXXXX
                    echo -n "Вы ўпэўнены, што хочаце выдаліць карыстальніка {selected_option} з вашай сістэмы? \nКалі ваш адказ >>ТАК<<, то выбраны карыстальнік і ўсе звязаныя з ім дадзеныя будуць выдалены з гэтай сістэмы!" > /tmp/_selected_del_user_warn_text.XXXXXXX
                    echo -n "Абраны карыстальнік {selected_option} не можа быць выдалены, таму што вы ўвайшлі ў сістэму з яго дапамогай! \nКалі ласка, выберыце іншага карыстальніка, калі вы хочаце працягнуць выдаленне карыстальнікаў, якія больш не патрэбныя ў гэтай сістэме." > /tmp/_selected_del_user_info_text.XXXXXXX
               """
...
class MainWindow_No_Del_Selected_User_Info(Gtk.Window):
    def __init__(self):
        ...
        label_1 = Gtk.Label(label="Вы не выбралі карыстальніка для выдалення! \nКалі ласка, абярыце карыстальніка, перш чым працягнуць.")
        ...
        # Add a "Okay" button
        button_okay = Gtk.Button.new_with_label("◀️ Назад")
...
class MainWindow_No_Configure_Selected_User_Info(Gtk.Window):
    def __init__(self):
        ...
        label_1 = Gtk.Label(label="Вы не выбралі карыстальніка для канфігурацыі! \nКалі ласка, абярыце карыстальніка, перш чым працягнуць.")
        label_1.set_justify(Gtk.Justification.CENTER)
        vbox.pack_start(label_1, True, True, 0)
...
class Window_Create_User(Gtk.Window):
    def __init__(self):
        ...
        label_title = Gtk.Label()
        label_title.set_markup(
            "<big><b>Стварыць уліковы запіс</b></big>"
        )
        label_title.set_justify(Gtk.Justification.CENTER)
        info_box.pack_start(label_title, True, True, 0)
        label_info = Gtk.Label(label="Калі ласка, запоўніце наступныя дадзеныя, каб стварыць новы ўліковы запіс.")
        label_info.set_justify(Gtk.Justification.CENTER)
        info_box.pack_start(label_info, True, True, 0)      
        ...
        fullname_label = Gtk.Label("Поўнае імя:             ")
        ...
        username_label = Gtk.Label("Імя карыстальніка:")
        ...
        confirm_password_label = Gtk.Label("Пацверджанне:      ")
        ...
        show_password_checkbox = Gtk.CheckButton(" Паказаць пароль")
        ...
        label = Gtk.Label(label="|   Аўтаматычны ўваход:")
...
class Window_Create_User_Info_Completed(Gtk.Window):
    def __init__(self):
        ...
        label_1 = Gtk.Label(label="Уліковы запіс карыстальніка быў паспяхова створаны, і вы можаце выкарыстоўваць гэтага новага карыстальніка пасля паўторнага ўваходу!")
        label_1.set_justify(Gtk.Justification.CENTER)
        vbox.pack_start(label_1, True, True, 0)
...
class Window_Create_User_Error_1(Gtk.Window):
    def __init__(self):
        ...
        label_1 = Gtk.Label(label="Новы пароль павінен быць не менш за 8 знакаў!")
        label_1.set_justify(Gtk.Justification.CENTER)
        vbox.pack_start(label_1, True, True, 0)
...
class Window_Create_User_Error_2(Gtk.Window):
    def __init__(self):
        ...
        label_1 = Gtk.Label(label="Паролі ўліковых запісаў карыстальнікаў не супадаюць. Калі ласка, паспрабуйце яшчэ раз!")
        label_1.set_justify(Gtk.Justification.CENTER)
        vbox.pack_start(label_1, True, True, 0)
...
class Window_Configure_User(Gtk.Window):
    ...
    def __init__(self):
        ...
        label_title = Gtk.Label()
        label_title.set_markup(
            "<big><b>Наладзьце ўліковы запіс</b></big>"
        )
        label_title.set_justify(Gtk.Justification.CENTER)
        info_box.pack_start(label_title, True, True, 0)
        label_info = Gtk.Label(label="У гэтай вобласці вы можаце змяніць налады ўліковага запісу карыстальніка.")
        label_info.set_line_wrap(True)
        label_info.set_max_width_chars(55)
        label_info.set_justify(Gtk.Justification.CENTER)
        info_box.pack_start(label_info, True, True, 0)
        ...
        username_box.set_valign(Gtk.Align.CENTER)
        main_box.pack_start(username_box, False, False, 0)
        username_label = Gtk.Label("Выбранае імя карыстальніка:")
        username_box.pack_start(username_label, False, False, 0)
        self.username_entry = Gtk.Entry()
        ...
        old_password_label = Gtk.Label("Бягучы пароль:                        ")
        ...
        new_password_label = Gtk.Label("Новы пароль:                           ")
        ...
        new_confirm_password_label = Gtk.Label("Пацвердзіце новы пароль:    ")
        ...
        change_user_groups_button = Gtk.Button("⚙️ Наладзьце групы")
        ...
        save_button = Gtk.Button("💾 Захаваць")
...
class Window_Configure_User_Groups(Gtk.Window):
    def __init__(self):
        ...
        label_title = Gtk.Label()
        label_title.set_markup(
            "<big><b>Наладзьце групы</b></big>"
        )
        label_title.set_justify(Gtk.Justification.CENTER)
        info_box.pack_start(label_title, True, True, 0)
        label_info = Gtk.Label(label="У гэтай вобласці вы можаце ўбачыць, да якіх груп належыць выбраны карыстальнік, а таксама змяніць членства ў групе. \n\n"
             "Але будзьце асцярожныя! Вы павінны разумець, што вы павінны дакладна ведаць, што вы робіце. Гуляючы і выпрабоўваючы дазволы карыстальнікаў і груп, вы можаце спыніць усю вашу сістэму! \n\n"
             "Калі вы хочаце стварыць або выдаліць карыстальніка, вы проста павінны мець аднаго больш ці менш карыстальнікаў. "
             "Аднак, калі вы даяце карыстальніку больш правоў або парушаеце правы сістэмнага ўліковага запісу або сістэмнай групы, вы можаце паставіць пад пагрозу бяспеку вашай сістэмы!")
        label_info.set_line_wrap(True)
        label_info.set_max_width_chars(55)
        label_info.set_justify(Gtk.Justification.CENTER)
        info_box.pack_start(label_info, True, True, 0)
        ...
        column_toggle = Gtk.TreeViewColumn("Статус", renderer_toggle, active=1)
        self.treeview.append_column(column_toggle)

        renderer_text = Gtk.CellRendererText()
        column_text = Gtk.TreeViewColumn("Група карыстальнікаў", renderer_text, text=0)
...
class Window_Configure_User_Info_Completed(Gtk.Window):
    def __init__(self):
        ...
        label_1 = Gtk.Label(label="Уліковы запіс карыстальніка быў паспяхова выдалены з усімі звязанымі данымі!")
        label_1.set_justify(Gtk.Justification.CENTER)
        vbox.pack_start(label_1, True, True, 0)
...
class Window_Configure_User_Error_1(Gtk.Window):
    def __init__(self):
        ...
        label_1 = Gtk.Label(label="Стары пароль уліковага запісу не павінен супадаць з новым! \nКалі ласка, абярыце іншы пароль.")
        label_1.set_justify(Gtk.Justification.CENTER)
        vbox.pack_start(label_1, True, True, 0)
...
class Window_Configure_User_Error_3(Gtk.Window):
    def __init__(self):
        ...
        label_1 = Gtk.Label(label="Новы пароль і паўторна ўведзены пароль не супадаюць!")
        label_1.set_justify(Gtk.Justification.CENTER)
        vbox.pack_start(label_1, True, True, 0)
