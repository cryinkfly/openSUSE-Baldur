...
class MainWindow(Gtk.Window):
    def __init__(self):
        ...
        label_title.set_markup(
            "<big><b>إعدادات مدير المستخدم</b></big>"
        )
        label_title.set_justify(Gtk.Justification.CENTER)
        info_box.pack_start(label_title, True, True, 0)
        label_info = Gtk.Label(label="في هذه المنطقة، يمكنك تغيير إعدادات حساب المستخدم وإضافة/حذف المستخدمين المرتبطين بحساباتهم. ما عليك سوى تحديد المستخدم الذي تريد تعديله من القائمة.")
        ...
        column_toggle = Gtk.TreeViewColumn("يختار", renderer_toggle, active=0)
        treeview.append_column(column_toggle)

        renderer_text = Gtk.CellRendererText()
        column_text = Gtk.TreeViewColumn("مستخدم", renderer_text, text=1)
        treeview.append_column(column_text)

        # Buttons
        close_button = Gtk.Button(label="◀️ خلف")
        close_button.connect("clicked", self.on_close_clicked)

        add_button = Gtk.Button(label="🎭 يخلق")
        add_button.connect("clicked", self.on_add_clicked)

        configure_button = Gtk.Button(label="⚙️ تهيئة")
        configure_button.connect("clicked", self.on_configure_clicked)

        del_button = Gtk.Button(label="🗑 يمسح")
    ...
    def on_del_clicked(self, button):
        ...
        if selected_option:
            del_selected_user_cmd=f"""
                    ...
                    echo -n {selected_option} > /tmp/_selected_user.XXXXXXX
                    echo -n "هل أنت متأكد أنك تريد إزالة المستخدم {selected_option} من نظامك؟ \nإذا كانت إجابتك >>نعم<<، فسيتم حذف المستخدم المحدد وجميع البيانات المرتبطة به من هذا النظام!" > /tmp/_selected_del_user_warn_text.XXXXXXX
                    echo -n "لا يمكن حذف المستخدم المحدد {selected_option} لأنك قمت بتسجيل الدخول إلى هذا النظام به! \nالرجاء تحديد مستخدم آخر إذا كنت تريد الاستمرار في حذف المستخدمين الذين لم تعد هناك حاجة إليهم على هذا النظام." > /tmp/_selected_del_user_info_text.XXXXXXX
               """
...
class MainWindow_No_Del_Selected_User_Info(Gtk.Window):
    def __init__(self):
        ...
        label_1 = Gtk.Label(label="لم تقم بتحديد مستخدم لحذفه! \nالرجاء تحديد مستخدم قبل المتابعة.")
        ...
        # Add a "Okay" button
        button_okay = Gtk.Button.new_with_label("◀️ خلف")
...
class MainWindow_No_Configure_Selected_User_Info(Gtk.Window):
    def __init__(self):
        ...
        label_1 = Gtk.Label(label="لم تقم بتحديد مستخدم لتكوينه! \nالرجاء تحديد مستخدم قبل المتابعة.")
        label_1.set_justify(Gtk.Justification.CENTER)
        vbox.pack_start(label_1, True, True, 0)
...
class Window_Create_User(Gtk.Window):
    def __init__(self):
        ...
        label_title = Gtk.Label()
        label_title.set_markup(
            "<big><b>إنشاء حساب</b></big>"
        )
        label_title.set_justify(Gtk.Justification.CENTER)
        info_box.pack_start(label_title, True, True, 0)
        label_info = Gtk.Label(label="يرجى ملء التفاصيل التالية لإنشاء حساب جديد.")
        label_info.set_justify(Gtk.Justification.CENTER)
        info_box.pack_start(label_info, True, True, 0)      
        ...
        fullname_label = Gtk.Label("الاسم الكامل:   ")
        ...
        username_label = Gtk.Label("اسم المستخدم:")
        ...
        password_label = Gtk.Label("كلمة المرور:    ")
        ...
        confirm_password_label = Gtk.Label("تأكيد:               ")
        ...
        show_password_checkbox = Gtk.CheckButton("عرض كلمة المرور ")
        ...
        show_password_autologin_box.pack_start(autologin_switch, False, False, 0)
        label1 = Gtk.Label(label="تسجيل الدخول التلقائي:   ")
        box_autologin.pack_start(label1, True, True, 0)
        show_password_autologin_box.pack_start(box_autologin, False, False, 0)
...
class Window_Create_User_Info_Completed(Gtk.Window):
    def __init__(self):
        ...
        label_1 = Gtk.Label(label="تم إنشاء حساب المستخدم بنجاح ويمكنك استخدام هذا المستخدم الجديد بعد إعادة تسجيل الدخول!")
        label_1.set_justify(Gtk.Justification.CENTER)
        vbox.pack_start(label_1, True, True, 0)
...
class Window_Create_User_Error_1(Gtk.Window):
    def __init__(self):
        ...
        label_1 = Gtk.Label(label="يجب أن تتكون كلمة المرور الجديدة من 8 أحرف على الأقل!")
        label_1.set_justify(Gtk.Justification.CENTER)
        vbox.pack_start(label_1, True, True, 0)
...
class Window_Create_User_Error_2(Gtk.Window):
    def __init__(self):
        ...
        label_1 = Gtk.Label(label="كلمات مرور حساب المستخدم غير متطابقة. حاول مرة اخرى!")
        label_1.set_justify(Gtk.Justification.CENTER)
        vbox.pack_start(label_1, True, True, 0)
...
class Window_Configure_User(Gtk.Window):
    ...
    def __init__(self):
        ...
        label_title = Gtk.Label()
        label_title.set_markup(
            "<big><b>تكوين حساب</b></big>"
        )
        label_title.set_justify(Gtk.Justification.CENTER)
        info_box.pack_start(label_title, True, True, 0)
        label_info = Gtk.Label(label="في هذه المنطقة يمكنك تغيير إعدادات حساب المستخدم.")
        label_info.set_line_wrap(True)
        label_info.set_max_width_chars(55)
        label_info.set_justify(Gtk.Justification.CENTER)
        info_box.pack_start(label_info, True, True, 0)
        ...
        username_box.pack_start(self.username_entry, False, False, 0)
        username_label = Gtk.Label("اسم المستخدم المحدد:    ")
        username_box.pack_start(username_label, False, False, 0)
        ...
        old_password_label = Gtk.Label("كلمة السر الحالية:            ")
        ...
        new_password_label = Gtk.Label("كلمة المرور الجديدة:        ")
        ...
        new_confirm_password_label = Gtk.Label("تأكيد كلمة المرور الجديدة:")
        ...
        box_autologin.pack_start(label1, True, True, 0)
        label = Gtk.Label(label="|")
        show_password_autologin_box.pack_start(label, True, True, 0)
        show_password_autologin_box.pack_start(box_autologin, False, False, 0)
        ...
        change_user_groups_button = Gtk.Button("⚙️ تكوين المجموعات")
...
class Window_Configure_User_Groups(Gtk.Window):
    def __init__(self):
        ...
        label_title = Gtk.Label()
        label_title.set_markup(
            "<big><b>تكوين المجموعات</b></big>"
        )
        label_title.set_justify(Gtk.Justification.CENTER)
        info_box.pack_start(label_title, True, True, 0)
        label_info = Gtk.Label(label="في هذه المنطقة، يمكنك معرفة المجموعات التي ينتمي إليها المستخدم الذي حددته ويمكنك أيضًا تغيير عضوية المجموعة. \n\n"
            "لكن كن حذرا! عليك أن تدرك أنه يجب عليك أن تعرف بالضبط ما تفعله. إن اللعب وتجربة أذونات المستخدم والمجموعة يمكن أن يؤدي إلى توقف النظام بأكمله! \n\n"
            "إذا كنت تريد إنشاء مستخدم أو حذفه، فما عليك سوى أن يكون لديك مستخدم واحد أكثر أو أقل. "
            "ومع ذلك، إذا منحت مستخدمًا المزيد من الحقوق أو خالفت حقوق حساب النظام أو مجموعة النظام، فقد تعرض أمان نظامك للخطر!")
        label_info.set_line_wrap(True)
        label_info.set_max_width_chars(55)
        label_info.set_justify(Gtk.Justification.CENTER)
        info_box.pack_start(label_info, True, True, 0)
        ...
        column_toggle = Gtk.TreeViewColumn("حالة", renderer_toggle, active=1)
        self.treeview.append_column(column_toggle)

        renderer_text = Gtk.CellRendererText()
        column_text = Gtk.TreeViewColumn("مجموعة المستخدمين", renderer_text, text=0)
...
class Window_Configure_User_Info_Completed(Gtk.Window):
    def __init__(self):
        ...
        label_1 = Gtk.Label(label="تم حذف حساب المستخدم بنجاح مع كافة البيانات المرتبطة به!")
        label_1.set_justify(Gtk.Justification.CENTER)
        vbox.pack_start(label_1, True, True, 0)
...
class Window_Configure_User_Error_1(Gtk.Window):
    def __init__(self):
        ...
        label_1 = Gtk.Label(label="يجب ألا تتطابق كلمة مرور الحساب القديم مع كلمة المرور الجديدة! \nالرجاء اختيار كلمة مرور مختلفة.")
        label_1.set_justify(Gtk.Justification.CENTER)
        vbox.pack_start(label_1, True, True, 0)
...
class Window_Configure_User_Error_3(Gtk.Window):
    def __init__(self):
        ...
        label_1 = Gtk.Label(label="كلمة المرور الجديدة وكلمة المرور الجديدة التي تم إدخالها بشكل متكرر غير متطابقين!")
        label_1.set_justify(Gtk.Justification.CENTER)
        vbox.pack_start(label_1, True, True, 0)
